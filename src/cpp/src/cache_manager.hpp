...
public:
    explicit CacheManager(const DeviceConfig &device_config, ov::Core core) :
        ...
        if (device_name.find("GPU") == std::string::npos) {// Allocate KV caches
            for (size_t decoder_layer_id = 0; decoder_layer_id < m_device_config.get_num_layers(); ++decoder_layer_id) {
                ...
                std::cout << "GenAI|decoder_layer_id|" << decoder_layer_id << value_cache.get_element_type() << "|byte_size|" << value_cache.get_byte_size() << std::endl;
    ...
    void copy_blocks(const std::map<size_t, std::list<size_t>>& block_copy_map) {
        ...
        std::cout << "GenAI|block_pre|" << block_copy_map.size() << "|key_src_end_roi|" << key_src_start_roi
                  << "|key_src_end_roi|" << key_src_end_roi << std::endl;
        std::cout << "GenAI|block_pre|" << block_copy_map.size() << "|key_dst_cache_roi|" << key_dst_start_roi
                  << "|key_dst_end_roi|" << key_dst_end_roi << std::endl;
        std::cout << "GenAI|block_pre|" << block_copy_map.size() << "|value_src_end_roi|" << value_src_start_roi
                  << "|value_src_end_roi|" << value_src_end_roi << std::endl;
        std::cout << "GenAI|block_pre|" << block_copy_map.size() << "|value_dst_cache_roi|" << value_dst_start_roi
                  << "|value_dst_end_roi|" << value_dst_end_roi << std::endl;
        for (const auto & blocks_pair : block_copy_map) {
            ...
            for (size_t dst_block_id : dst_block_ids) {
                ...
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_device_config.get_num_layers(); ++decoder_layer_id) {
                    ...
                    //     std::cout << "GenAI|decoder_layer_id" << "|" << decoder_layer_id << "|key_start|"
                    //             << key_src_start_roi[0] << "|key_end|" << key_dst_start_roi[0] << std::endl;
                    //     std::cout << "GenAI|decoder_layer_id" << "|" << decoder_layer_id << "|value_start|"
                    //             << value_src_start_roi[0] << "|value_end|" << value_dst_start_roi[0] << std::endl;
                    // }
                    if (m_value_cache[decoder_layer_id].get_element_type() != ov::element::u4 && getenv("USE_OV_COPY"))  {
                        ...
                        auto copy_data = [&](ov::Tensor& dst, const ov::Tensor& src) {
                            ...
                            // std::cout << "GenAI|Shape|" << value_shape << "|stride|" << stride << std::endl;
