...
class DeviceConfig {
    ...
    size_t m_key_cache_group_size;
    size_t m_value_cache_group_size;
...
public:
    DeviceConfig(ov::Core& core, const SchedulerConfig& scheduling_config, const std::string& device, const ov::AnyMap& plugin_config = {}) {
        ...
        if (m_device == "CPU") {
            ...
            if (m_key_cache_type == ov::element::u8 && getenv("ENABLE_VALUE_U4")) {
                m_value_cache_type = ov::element::u4;
            }
            if (m_key_cache_type == ov::element::u8 && getenv("ENABLE_VALUE_S4")) {
                m_value_cache_type = ov::element::i4;
            }

            auto key_group_size_it = plugin_config.find(ov::hint::key_cache_group_size.name());
            if (key_group_size_it != plugin_config.end()) {
                m_key_cache_group_size = key_group_size_it->second.as<size_t>();
            } else {
                m_key_cache_group_size = 0;
            }
            auto value_group_size_it = plugin_config.find(ov::hint::value_cache_group_size.name());
            if (value_group_size_it != plugin_config.end()) {
                m_value_cache_group_size = value_group_size_it->second.as<size_t>();
            } else {
    ...
    void set_model_params(size_t num_kv_heads, size_t head_size, size_t num_decoder_layers) {
        ...
        std::cout << "GenAI|" << "key_group_size|" << m_key_cache_group_size << "|value_group_size|"
                  << m_value_cache_group_size << std::endl;
        ...
        if (m_device == "CPU") {
            ...
            auto init_cache_shape = [&](ov::element::Type precision, size_t cache_group_size) {
                size_t head_size = m_head_size;
                if (m_head_size % cache_group_size != 0)
                    throw std::runtime_error("head_size cannot be divided by group_size");
                size_t group_num = m_head_size / cache_group_size;
                if (precision == ov::element::u8) {
                    std::cout << "GenAI|group_size|" << cache_group_size << "|group_num|" << group_num << std::endl;
